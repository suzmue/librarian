...
func Generate(ctx context.Context, cfg *config.Config, library *config.Library, sources *sources.Sources) error {
	...
	if library.Rust != nil && len(library.Rust.AnyFields) > 0 {
		if err := loadAnyFields(library, sources, pc, modelConfig, model); err != nil {
			return err
		}
	}
...
func loadAnyFields(library *config.Library, srcs *sources.Sources, pc *config.Protoc, modelConfig *parser.ModelConfig, mainModel *api.API) error {
