...
func Generate(ctx context.Context, cfg *config.Config, library *config.Library, sources *sources.Sources) error {
	...
	if library.Rust != nil && library.Rust.IncludeStreamingMethods {
		if err := generateStreamingSupport(ctx, library, sources); err != nil {
			return err
		}
	}
...
func generateVeneer(ctx context.Context, library *config.Library, sources *sources.Sources) error {
	...
	for _, module := range library.Rust.Modules {
		...
		if module.Template == "convert-prost" {
			appendExtraMessagesAndEnums(model)
		}
...
type modNode struct {
	Name     string
	Children map[string]*modNode
	File     string
}

func renderNode(node *modNode, indent string) string {
	var sb strings.Builder
	fmt.Fprintf(&sb, "%spub mod %s {\n", indent, node.Name)
	if node.File != "" {
		fmt.Fprintf(&sb, "%s    include!(\"prost/%s\");\n", indent, node.File)
	}
	if len(node.Children) > 0 {
		...
		sort.Strings(keys)
		for _, k := range keys {
			sb.WriteString(renderNode(node.Children[k], indent+"    "))
		}
	}
	fmt.Fprintf(&sb, "%s}\n", indent)
	return sb.String()
...
func generateProstRsContent(prostDir string, mainPackageName string) (string, error) {
	files, err := os.ReadDir(prostDir)
	if err != nil {
		return "", err
	}

	rootNodes := make(map[string]*modNode)
	for _, f := range files {
		if f.IsDir() || !strings.HasSuffix(f.Name(), ".rs") {
			continue
		}
		name := strings.TrimSuffix(f.Name(), ".rs")
		parts := strings.Split(name, ".")
		currNodes := rootNodes
		for i, part := range parts {
			node, ok := currNodes[part]
			if !ok {
				node = &modNode{
					Name:     part,
					Children: make(map[string]*modNode),
				}
				currNodes[part] = node
			}
			if i == len(parts)-1 {
	...
	var sb strings.Builder
	sb.WriteString("// Code generated by librarian. DO NOT EDIT.\n\n")
	var keys []string
	...
	sort.Strings(keys)
	for _, k := range keys {
		sb.WriteString(renderNode(rootNodes[k], ""))
	}
	mainParts := strings.Split(mainPackageName, ".")
	fmt.Fprintf(&sb, "#[allow(unused_imports)]\npub use %s::*;\n", strings.Join(mainParts, "::"))
	return sb.String(), nil
}

func generateStreamingSupport(ctx context.Context, library *config.Library, sources *sources.Sources) error {
	prostModule := &config.RustModule{
		Template:                "prost",
		Output:                  filepath.Join(library.Output, "src", "prost"),
		APIPath:                 library.APIs[0].Path,
		IncludeStreamingMethods: true,
	}
	prostModelConfig, err := moduleToModelConfig(library, prostModule, sources)
	if err != nil {
		return fmt.Errorf("failed to create prost model config: %w", err)
	}
	prostModel, err := parser.CreateModel(prostModelConfig)
	if err != nil {
		return fmt.Errorf("failed to create prost model: %w", err)
	}
	if err := os.MkdirAll(prostModule.Output, 0755); err != nil {
		return fmt.Errorf("failed to create prost output directory: %w", err)
	}
	if err := rust_prost.Generate(ctx, prostModel, prostModule.Output, prostModule.Template, prostModelConfig); err != nil {
		return fmt.Errorf("failed to generate prost module: %w", err)
	}

	prostRsPath := filepath.Join(library.Output, "src", "prost.rs")
	prostRsContent, err := generateProstRsContent(prostModule.Output, prostModel.PackageName)
	if err != nil {
		return fmt.Errorf("failed to generate prost.rs content: %w", err)
	}
	if err := os.WriteFile(prostRsPath, []byte(prostRsContent), 0644); err != nil {
		return fmt.Errorf("failed to write prost.rs: %w", err)
	}

	convertProstModule := &config.RustModule{
		Template:                "convert-prost",
		Output:                  filepath.Join(library.Output, "src"),
		APIPath:                 library.APIs[0].Path,
		IncludeStreamingMethods: true,
	}
	convertProstModelConfig, err := moduleToModelConfig(library, convertProstModule, sources)
	if err != nil {
		return fmt.Errorf("failed to create convert-prost model config: %w", err)
	}
	convertProstModel, err := parser.CreateModel(convertProstModelConfig)
	if err != nil {
		return fmt.Errorf("failed to create convert-prost model: %w", err)
	}

	appendExtraMessagesAndEnums(convertProstModel)

	if err := sidekickrust.Generate(ctx, convertProstModel, convertProstModule.Output, convertProstModelConfig); err != nil {
		return fmt.Errorf("failed to generate convert-prost module: %w", err)
	}
	return nil
}

func appendExtraMessagesAndEnums(model *api.API) {
	var extraMessages []*api.Message
	for m := range model.AllMessages() {
		if m.Parent != nil {
			continue
		}
		if m.Package == model.PackageName {
			continue
		}
		if strings.HasPrefix(m.Package, "google.protobuf") || strings.HasPrefix(m.Package, "google.api") {
			continue
		}
		found := false
		for _, existing := range model.Messages {
			if existing.ID == m.ID {
				found = true
				break
			}
	...
	sort.Slice(extraMessages, func(i, j int) bool {
		return extraMessages[i].ID < extraMessages[j].ID
	})
	model.Messages = append(model.Messages, extraMessages...)

	var extraEnums []*api.Enum
	for e := range model.AllEnums() {
		if e.Parent != nil {
			continue
		}
		if e.Package == model.PackageName {
			continue
		}
		if strings.HasPrefix(e.Package, "google.protobuf") || strings.HasPrefix(e.Package, "google.api") {
			continue
		}
		found := false
		for _, existing := range model.Enums {
			if existing.ID == e.ID {
				found = true
				break
			}
		...
		if !found {
			extraEnums = append(extraEnums, e)
		}
	}
	sort.Slice(extraEnums, func(i, j int) bool {
		return extraEnums[i].ID < extraEnums[j].ID
	})
	model.Enums = append(model.Enums, extraEnums...)
}
